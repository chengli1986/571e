...
main(int argc, char **argv)
{
    ...
    if (g_bQAReadback)
    {
        ...
        if (!sdkCompareBin2BinFloat("smokeParticles_pos.bin", sRefBin[0], numParticles*sizeof(float4),
                                    MAX_EPSILON_ERROR, THRESHOLD, argv[0]))
        {
            g_TotalErrors++;
        }

        if (!sdkCompareBin2BinFloat("smokeParticles_vel.bin", sRefBin[1], numParticles*sizeof(float4),
                                    MAX_EPSILON_ERROR, THRESHOLD, argv[0]))
        {
            g_TotalErrors++;
        }
