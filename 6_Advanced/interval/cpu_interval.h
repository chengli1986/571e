...
class global_stack_cpu
{
    ...
    public:
        // buf should point to an allocated global buffer of size N * THREADS * sizeof(T)
...
template<class I>
bool should_bisect_cpu(I const &x, I const &x1, I const &x2, typename I::base_type alpha)
{
    typedef typename I::base_type T;
...
// Always keep the next interval to work on in registers (avoids excessive spilling to local mem)
...
bool checkAgainstHost(int *h_nresults, int *h_nresults_cpu, I_CPU *h_result, I_CPU *h_result_cpu)
{
    ...
    if (h_nresults_cpu[0] == h_nresults[0])
    {
        ...
        for (int i = 0; i != h_nresults[0]; ++i)
        {
            TYPE diff1 = abs(h_result[THREADS * i + 0].lower() - h_result_cpu[THREADS * (h_nresults[0]-i-1) + 0].lower());
            TYPE diff2 = abs(h_result[THREADS * i + 0].upper() - h_result_cpu[THREADS * (h_nresults[0]-i-1) + 0].upper());
